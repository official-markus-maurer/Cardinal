...
static CardinalLogLevel s_min_log_level =
    CARDINAL_LOG_LEVEL_WARN; // Default to WARN to reduce spam
  ...
  std::call_once(s_spdlog_init_flag, [level]() {
    try {
      ...
      auto file_sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(
          "cardinal_log.txt", true);
      std::vector<spdlog::sink_ptr> sinks{console_sink, file_sink};
      s_logger = std::make_shared<spdlog::logger>("cardinal", sinks.begin(),
                                                  sinks.end());
      ...
      switch (level) {
      case CARDINAL_LOG_LEVEL_TRACE:
        lvl = spdlog::level::trace;
        break;
      case CARDINAL_LOG_LEVEL_DEBUG:
        lvl = spdlog::level::debug;
        break;
      case CARDINAL_LOG_LEVEL_INFO:
        lvl = spdlog::level::info;
        break;
      case CARDINAL_LOG_LEVEL_WARN:
        lvl = spdlog::level::warn;
        break;
      case CARDINAL_LOG_LEVEL_ERROR:
        lvl = spdlog::level::err;
        break;
      case CARDINAL_LOG_LEVEL_FATAL:
 ...
 */
void cardinal_log_init(void) { cardinal_log_init_with_level(s_min_log_level); }

/**
 * @brief Initializes logging with specified minimum level.
 * @param min_level Minimum level to log.
 *
 * @todo Integrate Vulkan debug utils extension (VK_EXT_debug_utils) for
 * GPU-side logging.
 */
void cardinal_log_init_with_level(CardinalLogLevel min_level) {
  ...
  if (s_logger) {
    s_logger->info("==== Cardinal Log Start (Level: {}) ====",
                   level_str(min_level));
  }
}
 ...
 */
CardinalLogLevel cardinal_log_get_level(void) { return s_min_log_level; }
...
CardinalLogLevel cardinal_log_parse_level(const char *level_str_input) {
  ...
  if (strcmp(level_str_input, "TRACE") == 0 ||
      strcmp(level_str_input, "trace") == 0)
    return CARDINAL_LOG_LEVEL_TRACE;
  if (strcmp(level_str_input, "DEBUG") == 0 ||
      strcmp(level_str_input, "debug") == 0)
    return CARDINAL_LOG_LEVEL_DEBUG;
  if (strcmp(level_str_input, "INFO") == 0 ||
      strcmp(level_str_input, "info") == 0)
    return CARDINAL_LOG_LEVEL_INFO;
  if (strcmp(level_str_input, "WARN") == 0 ||
      strcmp(level_str_input, "warn") == 0)
    return CARDINAL_LOG_LEVEL_WARN;
  if (strcmp(level_str_input, "ERROR") == 0 ||
      strcmp(level_str_input, "error") == 0)
    return CARDINAL_LOG_LEVEL_ERROR;
  if (strcmp(level_str_input, "FATAL") == 0 ||
      strcmp(level_str_input, "fatal") == 0)
...
void cardinal_log_output(CardinalLogLevel level, const char *file, int line,
  ...
  switch (level) {
  case CARDINAL_LOG_LEVEL_TRACE:
    s_logger->trace("{}:{}: {}", file, line, msgbuf);
    break;
  case CARDINAL_LOG_LEVEL_DEBUG:
    s_logger->debug("{}:{}: {}", file, line, msgbuf);
    break;
  case CARDINAL_LOG_LEVEL_INFO:
    s_logger->info("{}:{}: {}", file, line, msgbuf);
    break;
  case CARDINAL_LOG_LEVEL_WARN:
    s_logger->warn("{}:{}: {}", file, line, msgbuf);
    break;
  case CARDINAL_LOG_LEVEL_ERROR:
    s_logger->error("{}:{}: {}", file, line, msgbuf);
    break;
  case CARDINAL_LOG_LEVEL_FATAL:
    s_logger->critical("{}:{}: {}", file, line, msgbuf);
    break;
  }
