...
static void lerp_vector(const float* a, const float* b, float t, uint32_t component_count,
                        float* result) {
...
bool cardinal_animation_interpolate(CardinalAnimationInterpolation interpolation, float time,
    ...
    switch (interpolation) {
        ...
        case CARDINAL_ANIMATION_INTERPOLATION_CUBICSPLINE:
            cubic_spline_interpolate(output, prev_index, next_index, factor, component_count,
                                     result);
            break;
...
CardinalAnimationSystem* cardinal_animation_system_create(uint32_t max_animations,
                                                          uint32_t max_skins) {
    CardinalAnimationSystem* system =
        (CardinalAnimationSystem*)calloc(1, sizeof(CardinalAnimationSystem));
    if (!system) {
    ...
    // Allocate bone matrices buffer (256 matrices max)
    // We allocate this even if there are no skins yet, as the renderer might expect it
    // Initialize to identity matrices
    system->bone_matrix_count = 256; // Standard max bones
    system->bone_matrices = (float*)malloc(system->bone_matrix_count * 16 * sizeof(float));
    if (system->bone_matrices) {
        for (uint32_t i = 0; i < system->bone_matrix_count; ++i) {
            float* matrix = &system->bone_matrices[i * 16];
        ...
        CARDINAL_LOG_ERROR("Failed to allocate bone matrices buffer");
...
uint32_t cardinal_animation_system_add_animation(CardinalAnimationSystem* system,
    ...
    if (animation->sampler_count > 0) {
        dest->samplers = (CardinalAnimationSampler*)calloc(animation->sampler_count,
                                                           sizeof(CardinalAnimationSampler));
        if (dest->samplers) {
            for (uint32_t i = 0; i < animation->sampler_count; ++i) {
                ...
                if (src_sampler->input && src_sampler->input_count > 0) {
                    ...
                    if (dst_sampler->input) {
                        memcpy(dst_sampler->input, src_sampler->input,
                               src_sampler->input_count * sizeof(float));
                    }
                }
                ...
                if (src_sampler->output && src_sampler->output_count > 0) {
                    ...
                    if (dst_sampler->output) {
                        memcpy(dst_sampler->output, src_sampler->output,
                               src_sampler->output_count * sizeof(float));
                    }
                }
            }
        }
    ...
    if (animation->channel_count > 0) {
        dest->channels = (CardinalAnimationChannel*)malloc(animation->channel_count *
                                                           sizeof(CardinalAnimationChannel));
        if (dest->channels) {
            memcpy(dest->channels, animation->channels,
                   animation->channel_count * sizeof(CardinalAnimationChannel));
        }
    }

    system->animation_count++;
    CARDINAL_LOG_DEBUG("Added animation '%s' at index %u", dest->name ? dest->name : "Unnamed",
                       index);
    return index;
}
...
bool cardinal_animation_play(CardinalAnimationSystem* system, uint32_t animation_index, bool loop,
                             float blend_weight) {
    ...
    if (!state) {
        // Create new state
        system->states = (CardinalAnimationState*)realloc(
            system->states, (system->state_count + 1) * sizeof(CardinalAnimationState));
        if (!system->states) {
            return false;
        }
    ...
    CARDINAL_LOG_DEBUG("Started animation %u with blend weight %.2f", animation_index,
                       blend_weight);
    return true;
}
...
bool cardinal_animation_set_speed(CardinalAnimationSystem* system, uint32_t animation_index,
                                  float speed) {
    if (!system) {
        return false;
    }
...
void cardinal_animation_system_update(CardinalAnimationSystem* system,
                                      CardinalSceneNode** all_nodes, uint32_t all_node_count,
                                      float delta_time) {
    if (!system) {
        return;
    }
    ...
    for (uint32_t i = 0; i < system->state_count; ++i) {
        ...
        for (uint32_t c = 0; c < animation->channel_count; ++c) {
            ...
            CardinalAnimationSampler* sampler = &animation->samplers[channel->sampler_index];

            // Get target node
            if (!all_nodes || channel->target.node_index >= all_node_count) {
                continue;
            }
            CardinalSceneNode* node = all_nodes[channel->target.node_index];
            if (!node) {
                continue;
            }
            ...
            if (channel->target.path == CARDINAL_ANIMATION_TARGET_ROTATION ||
                channel->target.path ==
                    CARDINAL_ANIMATION_TARGET_WEIGHTS) { // Weights count varies but often 4
                component_count = 4;
            }

            if (cardinal_animation_interpolate(sampler->interpolation, state->current_time,
                                               sampler->input, sampler->output,
                                               sampler->input_count, component_count, result)) {
                // Apply to node transform
                // Note: We need to decompose current transform, update component, and recompose
                // But since we don't store TRS separately in node, this is tricky if we want to
                // blend For now, assume animation controls the property absolutely (no blending)

                // Better approach: Store TRS in node or re-extract it.
                // Or better yet, we assume the animation drives the local transform components.
                // We should ideally store base TRS in node.

                // Let's try to extract current TRS from local transform
                float translation[3], rotation[4], scale[3];
                cardinal_matrix_decompose(node->local_transform, translation, rotation, scale);

                switch (channel->target.path) {
                    case CARDINAL_ANIMATION_TARGET_TRANSLATION:
                        memcpy(translation, result, 3 * sizeof(float));
                        break;
                    case CARDINAL_ANIMATION_TARGET_ROTATION:
                        memcpy(rotation, result, 4 * sizeof(float));
                        break;
                    case CARDINAL_ANIMATION_TARGET_SCALE:
                        memcpy(scale, result, 3 * sizeof(float));
                        break;
                    default:
                        break;
                }
                ...
                cardinal_matrix_from_trs(translation, rotation, scale, new_transform);
                cardinal_scene_node_set_local_transform(node, new_transform);
            }
        }
    }
}
...
bool cardinal_skin_update_bone_matrices(const CardinalSkin* skin,
                                        const struct CardinalSceneNode** scene_nodes,
                                        float* bone_matrices) {
    ...
    for (uint32_t i = 0; i < skin->bone_count; ++i) {
        ...
        const float* world_transform =
            cardinal_scene_node_get_world_transform((struct CardinalSceneNode*)node);
