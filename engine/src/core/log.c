    ...
    std::call_once(s_spdlog_init_flag, [level]() {
        try {
            ...
            s_logger = std::make_shared<spdlog::async_logger>(
                "cardinal", s_runtime_sinks.begin(), s_runtime_sinks.end(), spdlog::thread_pool(),
...
void cardinal_log_output(CardinalLogLevel level, const char* file, int line, const char* fmt, ...) {
    ...
    snprintf(final_buffer, sizeof(final_buffer), "%s(%d): [%s] %s", filename, line,
    ...
    if (s_logger && s_spdlog_available) {
        ...
        switch (level) {
            case CARDINAL_LOG_LEVEL_TRACE:
                s_logger->trace(final_buffer);
                break;
            case CARDINAL_LOG_LEVEL_DEBUG:
                s_logger->debug(final_buffer);
                break;
            case CARDINAL_LOG_LEVEL_INFO:
                s_logger->info(final_buffer);
                break;
            case CARDINAL_LOG_LEVEL_WARN:
                s_logger->warn(final_buffer);
                break;
            case CARDINAL_LOG_LEVEL_ERROR:
                s_logger->error(final_buffer);
                break;
            case CARDINAL_LOG_LEVEL_FATAL:
                s_logger->critical(final_buffer);
                break;
        }
...
void cardinal_log_add_sink(void* sink_ptr) {
    if (!s_logger || !s_spdlog_available || !sink_ptr)
        return;
    ...
    s_logger = std::make_shared<spdlog::async_logger>("cardinal", s_runtime_sinks.begin(),
                                                      s_runtime_sinks.end(), spdlog::thread_pool(),
                                                      spdlog::async_overflow_policy::block);
    spdlog::register_logger(s_logger);
}
...
void cardinal_log_set_pattern(const char* pattern) {
    if (!s_logger || !s_spdlog_available || !pattern)
        return;
