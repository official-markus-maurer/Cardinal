    ...
    for (uint32_t m = 0; m < scene->mesh_count; m++) {
        ...
        for (uint32_t v = 0; v < mesh->vertex_count; v++) {
            ...
            if (first_vertex) {
                ...
                if (vertex->px < bbox_min[0])
                    bbox_min[0] = vertex->px;
                if (vertex->py < bbox_min[1])
                    bbox_min[1] = vertex->py;
                if (vertex->pz < bbox_min[2])
                    bbox_min[2] = vertex->pz;
                if (vertex->px > bbox_max[0])
                    bbox_max[0] = vertex->px;
                if (vertex->py > bbox_max[1])
                    bbox_max[1] = vertex->py;
                if (vertex->pz > bbox_max[2])
...
static bool expand_models_array(CardinalModelManager* manager) {
    uint32_t new_capacity =
        manager->model_capacity == 0 ? INITIAL_MODEL_CAPACITY : manager->model_capacity * 2;
...
static void rebuild_combined_scene(CardinalModelManager* manager) {
    ...
    manager->combined_scene.materials =
        (CardinalMaterial*)calloc(total_materials, sizeof(CardinalMaterial));
    manager->combined_scene.textures =
        (CardinalTexture*)calloc(total_textures, sizeof(CardinalTexture));

    if (!manager->combined_scene.meshes || !manager->combined_scene.materials ||
        !manager->combined_scene.textures) {
    ...
    for (uint32_t i = 0; i < manager->model_count; i++) {
        ...
        for (uint32_t m = 0; m < scene->mesh_count; m++) {
            ...
            if (!src_mesh->vertices || src_mesh->vertex_count == 0 || !src_mesh->indices ||
                src_mesh->index_count == 0 ||
                src_mesh->index_count > 1000000000) { // Sanity check for corrupted data
                CARDINAL_LOG_ERROR("Skipping corrupted mesh %u in model %u: vertices=%p, "
                                   "vertex_count=%u, indices=%p, index_count=%u",
                                   m, i, (void*)src_mesh->vertices, src_mesh->vertex_count,
                                   (void*)src_mesh->indices, src_mesh->index_count);
            ...
            *dst_mesh = *src_mesh;
            ...
            dst_mesh->vertices =
                (CardinalVertex*)malloc(src_mesh->vertex_count * sizeof(CardinalVertex));
            if (dst_mesh->vertices) {
                for (uint32_t v = 0; v < src_mesh->vertex_count; v++) {
                    dst_mesh->vertices[v] = src_mesh->vertices[v];

                    // Transform position
                    float position[3] = {src_mesh->vertices[v].px, src_mesh->vertices[v].py,
                                         src_mesh->vertices[v].pz};
                    float transformed[3];
                    ...
                    float normal[3] = {src_mesh->vertices[v].nx, src_mesh->vertices[v].ny,
                                       src_mesh->vertices[v].nz};
            ...
            if (src_mesh->indices && src_mesh->index_count > 0 &&
                src_mesh->index_count < 1000000000) {
                dst_mesh->indices = (uint32_t*)malloc(src_mesh->index_count * sizeof(uint32_t));
                if (dst_mesh->indices) {
                    memcpy(dst_mesh->indices, src_mesh->indices,
                           src_mesh->index_count * sizeof(uint32_t));
                } else {
                    CARDINAL_LOG_ERROR("Failed to allocate memory for indices in mesh %u", m);
                    dst_mesh->index_count = 0;
                }
            } else {
                CARDINAL_LOG_ERROR("Invalid indices data for mesh %u: indices=%p, count=%u", m,
                                   (void*)src_mesh->indices, src_mesh->index_count);
        ...
        if (scene->materials) {
            for (uint32_t mat = 0; mat < scene->material_count; mat++) {
                ...
                CardinalMaterial* dst_material =
                    &manager->combined_scene.materials[material_offset + mat];
                ...
                *dst_material = *src_material;
                ...
                if (src_material->ref_resource) {
                    if (src_material->ref_resource->identifier) {
                        // Check if we already have a reference (e.g. from previous rebuild)
                        // But here we are building a FRESH combined scene struct, so we need to
                        // acquire.
                        dst_material->ref_resource =
                            cardinal_ref_acquire(src_material->ref_resource->identifier);
                        if (!dst_material->ref_resource) {
                            CARDINAL_LOG_WARN("Failed to acquire ref for material %u (id: %s)", mat,
                                              src_material->ref_resource->identifier);
                        }
                    } else {
                        CARDINAL_LOG_WARN("Material %u has ref_resource %p but NULL identifier",
                                          mat, src_material->ref_resource);
        ...
        // Deep copy textures
        if (scene->textures) {
            // CARDINAL_LOG_DEBUG("Copying %u textures from model %u (offset: %u)",
            // scene->texture_count, i, texture_offset);
            for (uint32_t tex = 0; tex < scene->texture_count; tex++) {
                const CardinalTexture* src_texture = &scene->textures[tex];
                CardinalTexture* dst_texture =
                    &manager->combined_scene.textures[texture_offset + tex];

                // Copy texture structure
                *dst_texture = *src_texture;

                // Check if we can use reference counting to avoid deep copying data
                bool used_ref_counting = false;
                if (src_texture->ref_resource) {
                    // Validate identifier before access
                    if (src_texture->ref_resource->identifier) {
                        // Acquire reference to shared resource
                        dst_texture->ref_resource =
                            cardinal_ref_acquire(src_texture->ref_resource->identifier);
                        if (dst_texture->ref_resource) {
                            // Share data pointer (owned by the resource)
                            dst_texture->data = src_texture->data;
                            used_ref_counting = true;
                        } else {
                            CARDINAL_LOG_WARN("Failed to acquire ref for texture %u (id: %s), "
                                              "skipping data copy to avoid use-after-free [FIXED]",
                                              tex, src_texture->ref_resource->identifier);
                            dst_texture->ref_resource = NULL;
                            // IMPORTANT: Do NOT set used_ref_counting = false here.
                            // If we failed to acquire the ref, the source data is likely pointing
                            // to freed memory. We should NOT attempt to deep copy it.
                            dst_texture->data =
                                NULL; // Result will be a black/missing texture, but no crash.
                            used_ref_counting =
                                true; // Pretend we handled it to skip the deep copy block
                        }
                    } else {
                        dst_texture->ref_resource = NULL;
                    }
                ...
                if (!used_ref_counting) {
                    // No reference counting or failed to acquire - must deep copy data
                    dst_texture->ref_resource = NULL;

                    // Only deep copy if we have valid data and didn't fail a ref acquire above
                    // (The used_ref_counting=true in failure case handles this, but let's be extra
                    // safe)
                    if (src_texture->data && src_texture->width > 0 && src_texture->height > 0) {
                        CARDINAL_LOG_DEBUG("Deep copying data for texture %u (w=%u, h=%u, c=%u)",
                                           tex, src_texture->width, src_texture->height,
                                           src_texture->channels);
                        size_t data_size =
                            src_texture->width * src_texture->height * src_texture->channels;
                        dst_texture->data = (unsigned char*)malloc(data_size);
                        if (dst_texture->data) {
                            memcpy(dst_texture->data, src_texture->data, data_size);
                        } else {
                            CARDINAL_LOG_ERROR(
                                "Failed to allocate memory for texture copy (%zu bytes)",
                                data_size);
                        }
                    } else {
                ...
                // Deep copy path string (always needed as it's owned by CardinalTexture)
                if (src_texture->path) {
    ...
    CARDINAL_LOG_DEBUG("Rebuilt combined scene: %u meshes, %u materials, %u textures", total_meshes,
                       total_materials, total_textures);
}
...
uint32_t cardinal_model_manager_load_model(CardinalModelManager* manager, const char* file_path,
                                           const char* name) {
...
uint32_t cardinal_model_manager_add_scene(CardinalModelManager* manager, CardinalScene* scene,
                                          const char* file_path, const char* name) {
    if (!manager || !scene)
        return 0;
    ...
    model->name = name ? (char*)malloc(strlen(name) + 1)
                       : (file_path ? generate_model_name(file_path) : NULL);
    if (model->name && name) {
        strcpy(model->name, name);
    }
    ...
    model->scene = *scene;
    // Do NOT clear the source scene here, because we are taking ownership of the pointers.
    // If we clear the source, the caller might think it's still valid or we might lose references
    // if the struct was passed by value. However, since we're taking ownership, we should ensure
    // the caller doesn't try to free the resources.
    // The scene struct passed in is a local copy in scene_load_callback, so modifying it there
    // wouldn't affect the original data if it was allocated elsewhere, but here we are copying
    // the struct content. The pointers inside (meshes, materials, etc.) are now owned by
    // model->scene.

    // To be safe and explicit:
    memset(scene, 0, sizeof(CardinalScene));
...
bool cardinal_model_manager_remove_model(CardinalModelManager* manager, uint32_t model_id) {
    ...
    CARDINAL_LOG_INFO("Removing model '%s' (ID: %u)", model->name ? model->name : "Unnamed",
                      model_id);
...
CardinalModelInstance* cardinal_model_manager_get_model_by_index(CardinalModelManager* manager,
                                                                 uint32_t index) {
    if (!manager || index >= manager->model_count)
        return NULL;

    return &manager->models[index];
}

bool cardinal_model_manager_set_transform(CardinalModelManager* manager, uint32_t model_id,
...
bool cardinal_model_manager_set_visible(CardinalModelManager* manager, uint32_t model_id,
                                        bool visible) {
    if (!manager)
        return false;
...
void cardinal_model_manager_set_selected(CardinalModelManager* manager, uint32_t model_id) {
    ...
    if (manager->selected_model_id != 0) {
        CardinalModelInstance* prev_selected =
            cardinal_model_manager_get_model(manager, manager->selected_model_id);
        if (prev_selected) {
            ...
            if (status == CARDINAL_ASYNC_STATUS_COMPLETED) {
                ...
                if (cardinal_async_get_scene_result(model->load_task, &model->scene)) {
                    ...
                    CARDINAL_LOG_INFO("Async loading completed for model '%s' (ID: %u, %u meshes)",
                                      model->name ? model->name : "Unnamed", model->id,
                                      model->scene.mesh_count);
                } else {
                    CARDINAL_LOG_ERROR("Failed to get scene result for model '%s'",
