...
void cardinal_scene_node_update_transforms(CardinalSceneNode* node,
    ...
    if (node->world_transform_dirty || parent_world_transform) {
        if (parent_world_transform) {
            cardinal_matrix_multiply(parent_world_transform, node->local_transform,
...
const float* cardinal_scene_node_get_world_transform(CardinalSceneNode* node) {
    ...
    if (node->world_transform_dirty) {
        const float* parent_transform =
            node->parent ? cardinal_scene_node_get_world_transform(node->parent) : NULL;
...
void cardinal_scene_destroy(CardinalScene* scene) {
    ...
    if (scene->textures) {
        for (uint32_t i = 0; i < scene->texture_count; ++i) {
            ...
            // Always free path as it's owned by the texture struct, not the resource
