...
const vk_validation = @import("vulkan_validation.zig");
...
fn validation_enabled() bool {
    return vk_validation.validation_enabled();
}
...
fn get_instance_extensions(out_extensions: *[*c]const [*c]const u8, out_count: *u32, out_allocated: *bool) bool {
    ...
    const need_layer_settings = false;
...
fn configure_validation(ci: *c.VkInstanceCreateInfo, layers: [*]const [*c]const u8, debug_ci: *c.VkDebugUtilsMessengerCreateInfoEXT) void {
    vk_validation.configure_validation(ci, layers, debug_ci);
}
...
pub export fn vk_create_instance(s: ?*types.VulkanState) callconv(.c) bool {
    ...
    configure_validation(&ci, &layers, &debug_ci);
...
pub export fn vk_create_surface(s: ?*types.VulkanState, win: ?*window.CardinalWindow) callconv(.c) bool {
    ...
    const glfw_win: ?*c.GLFWwindow = @ptrCast(win.?.handle.?);

    const result = c.glfwCreateWindowSurface(vs.context.instance, glfw_win, null, &vs.context.surface);
...
pub export fn vk_recreate_debug_messenger(s: ?*types.VulkanState) callconv(.c) void {
    ...
    if (cfunc) |func| {
        ...
        vk_validation.fill_debug_messenger_create_info(&ci);
