...
static bool create_bindless_descriptor_pool(VkDevice device, uint32_t max_textures,
                                            VkDescriptorPool* out_pool) {
    VkDescriptorPoolSize pool_sizes[] = {
        {.type = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, .descriptorCount = max_textures},
        {      .type = VK_DESCRIPTOR_TYPE_SAMPLER, .descriptorCount = max_textures}
    };

    VkDescriptorPoolCreateInfo pool_info = {
        .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO,
        .flags = VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT,
        .maxSets = 100, // Increased from 1 to support multiple bindless sets
        .poolSizeCount = 2,
        .pPoolSizes = pool_sizes};

    CARDINAL_LOG_INFO("Creating bindless descriptor pool with %u max sets", pool_info.maxSets);
...
bool vk_bindless_texture_pool_init(BindlessTexturePool* pool, VulkanState* vulkan_state,
    ...
    if (!vk_descriptor_indexing_supported(vulkan_state)) {
        CARDINAL_LOG_ERROR(
            "Descriptor indexing not supported, cannot create bindless texture pool");
        return false;
    }

    memset(pool, 0, sizeof(BindlessTexturePool));

    pool->device = vulkan_state->context.device;
    pool->physical_device = vulkan_state->context.physical_device;
...
bool vk_bindless_texture_allocate(BindlessTexturePool* pool,
    ...
    texture->sampler = (create_info->custom_sampler != VK_NULL_HANDLE) ? create_info->custom_sampler
                                                                       : pool->default_sampler;
...
void vk_bindless_texture_free(BindlessTexturePool* pool, uint32_t texture_index) {
    ...
    if (!texture->is_allocated) {
        CARDINAL_LOG_WARN("Attempting to free already freed bindless texture at index %u",
                          texture_index);
        return;
    }
...
bool vk_bindless_texture_update_data(BindlessTexturePool* pool, uint32_t texture_index,
...
bool vk_bindless_texture_flush_updates(BindlessTexturePool* pool) {
    ...
    VkWriteDescriptorSet* writes =
        malloc(pool->pending_update_count * 2 * sizeof(VkWriteDescriptorSet));
    VkDescriptorImageInfo* image_infos =
        malloc(pool->pending_update_count * sizeof(VkDescriptorImageInfo));
    ...
    for (uint32_t i = 0; i < pool->pending_update_count; i++) {
        ...
        sampler_infos[i] = (VkDescriptorImageInfo){.imageLayout = VK_IMAGE_LAYOUT_UNDEFINED,
...
const BindlessTexture* vk_bindless_texture_get(const BindlessTexturePool* pool,
...
bool vk_descriptor_indexing_supported(const VulkanState* vulkan_state) {
    return vulkan_state && vulkan_state->context.supports_descriptor_indexing;
}

bool vk_create_variable_descriptor_layout(VkDevice device, uint32_t binding_count,
    ...
    VkDescriptorBindingFlags* binding_flags =
        calloc(binding_count, sizeof(VkDescriptorBindingFlags));
    if (!binding_flags) {
    ...
    binding_flags[variable_binding_index] = VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT |
...
bool vk_allocate_variable_descriptor_set(VkDevice device, VkDescriptorPool descriptor_pool,
                                         VkDescriptorSetLayout layout, uint32_t variable_count,
