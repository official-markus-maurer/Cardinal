...
bool cardinal_mt_cond_wait_timeout(cardinal_cond_t* cond, cardinal_mutex_t* mutex,
                                   uint32_t timeout_ms) {
    return SleepConditionVariableCS(cond, mutex, timeout_ms) != 0;
}

cardinal_thread_id_t cardinal_mt_get_current_thread_id(void) {
    return GetCurrentThreadId();
}

bool cardinal_mt_thread_ids_equal(cardinal_thread_id_t thread1, cardinal_thread_id_t thread2) {
...
static bool cardinal_mt_create_thread(cardinal_thread_handle_t* thread, void* arg) {
    *thread = (HANDLE)_beginthreadex(NULL, 0, cardinal_mt_worker_thread_func, arg, 0, NULL);
    return *thread != NULL;
}

static void cardinal_mt_join_thread(cardinal_thread_handle_t thread) {
...
static void cardinal_mt_task_queue_shutdown(CardinalMTTaskQueue* queue) {
    ...
    CardinalAllocator* allocator =
        cardinal_get_allocator_for_category(CARDINAL_MEMORY_CATEGORY_RENDERER);
    CardinalMTTask* current = queue->head;
    while (current) {
        CardinalMTTask* next = current->next;
        if (current->data) {
            // Note: data freeing policy needs to be defined.
            // Assuming data is managed by task creator or is simple alloc.
            // For string data in load tasks, it was malloc'd.
            if (current->type == CARDINAL_MT_TASK_TEXTURE_LOAD ||
                current->type == CARDINAL_MT_TASK_MESH_LOAD) {
                cardinal_free(allocator, current->data);
            }
        }
...
bool cardinal_mt_command_manager_init(CardinalMTCommandManager* manager,
...
void cardinal_mt_command_manager_shutdown(CardinalMTCommandManager* manager) {
    ...
    if (manager->vulkan_state && manager->vulkan_state->context.device != VK_NULL_HANDLE) {
        vkDeviceWaitIdle(manager->vulkan_state->context.device);
    }
    ...
    for (uint32_t i = 0; i < CARDINAL_MAX_MT_THREADS; i++) {
        ...
        if (pool->is_active) {
            if (pool->secondary_buffers) {
                cardinal_free(allocator, pool->secondary_buffers);
                pool->secondary_buffers = NULL;
            }

            if (pool->secondary_pool != VK_NULL_HANDLE) {
                vkDestroyCommandPool(manager->vulkan_state->context.device, pool->secondary_pool,
                                     NULL);
                pool->secondary_pool = VK_NULL_HANDLE;
            }

            if (pool->primary_pool != VK_NULL_HANDLE) {
                vkDestroyCommandPool(manager->vulkan_state->context.device, pool->primary_pool,
                                     NULL);
                pool->primary_pool = VK_NULL_HANDLE;
            }
...
CardinalThreadCommandPool* cardinal_mt_get_thread_command_pool(CardinalMTCommandManager* manager) {
    ...
    cardinal_thread_id_t current_thread = cardinal_mt_get_current_thread_id();
    ...
    VkCommandPoolCreateInfo primary_pool_info = {
        ...
        .queueFamilyIndex = manager->vulkan_state->context.graphics_queue_family};

    if (vkCreateCommandPool(manager->vulkan_state->context.device, &primary_pool_info, NULL,
                            &pool->primary_pool) != VK_SUCCESS) {
    ...
    if (vkCreateCommandPool(manager->vulkan_state->context.device, &secondary_pool_info, NULL,
                            &pool->secondary_pool) != VK_SUCCESS) {
        CARDINAL_LOG_ERROR("[MT] Failed to create secondary command pool for thread");
        vkDestroyCommandPool(manager->vulkan_state->context.device, pool->primary_pool, NULL);
        pool->primary_pool = VK_NULL_HANDLE;
        cardinal_mt_mutex_unlock(&manager->pool_mutex);
        return NULL;
    }
    ...
    CardinalAllocator* allocator =
        cardinal_get_allocator_for_category(CARDINAL_MEMORY_CATEGORY_RENDERER);
    pool->secondary_buffers = (VkCommandBuffer*)cardinal_alloc(
        allocator, sizeof(VkCommandBuffer) * pool->secondary_buffer_count);
    if (!pool->secondary_buffers) {
        CARDINAL_LOG_ERROR("[MT] Failed to allocate memory for secondary command buffers");
        vkDestroyCommandPool(manager->vulkan_state->context.device, pool->secondary_pool, NULL);
        vkDestroyCommandPool(manager->vulkan_state->context.device, pool->primary_pool, NULL);
        pool->primary_pool = VK_NULL_HANDLE;
        pool->secondary_pool = VK_NULL_HANDLE;
        cardinal_mt_mutex_unlock(&manager->pool_mutex);
    ...
    if (vkAllocateCommandBuffers(manager->vulkan_state->context.device, &alloc_info,
                                 pool->secondary_buffers) != VK_SUCCESS) {
    ...
    CARDINAL_LOG_INFO("[MT] Created command pool for thread (total active: %u)",
                      manager->active_thread_count);
...
bool cardinal_mt_allocate_secondary_command_buffer(CardinalThreadCommandPool* pool,
    ...
    if (!cardinal_mt_thread_ids_equal(pool->thread_id, current_thread)) {
        CARDINAL_LOG_ERROR(
            "[MT] Attempting to allocate from command pool owned by different thread");
        return false;
    }
...
bool cardinal_mt_begin_secondary_command_buffer(
    CardinalSecondaryCommandContext* context,
...
void cardinal_mt_execute_secondary_command_buffers(
    VkCommandBuffer primary_cmd, CardinalSecondaryCommandContext* secondary_contexts,
    uint32_t count) {
    ...
    VkCommandBuffer* secondary_buffers =
        (VkCommandBuffer*)cardinal_alloc(allocator, sizeof(VkCommandBuffer) * count);
    ...
    cardinal_free(allocator, secondary_buffers);
}
...
void cardinal_mt_process_completed_tasks(uint32_t max_tasks) {
    ...
    while ((max_tasks == 0 || processed < max_tasks)) {
        CardinalMTTask* task =
            cardinal_mt_task_queue_try_pop(&g_cardinal_mt_subsystem.completed_queue);
        if (!task)
            break;
        ...
        // Free the task data if it was internally allocated
        if (task->data) {
            if (task->type == CARDINAL_MT_TASK_TEXTURE_LOAD ||
                task->type == CARDINAL_MT_TASK_MESH_LOAD) {
                cardinal_free(allocator, task->data);
            }
        }
...
CardinalMTTask* cardinal_mt_create_texture_load_task(const char* file_path,
    ...
    CardinalAllocator* allocator =
        cardinal_get_allocator_for_category(CARDINAL_MEMORY_CATEGORY_RENDERER);
    CardinalMTTask* task = (CardinalMTTask*)cardinal_alloc(allocator, sizeof(CardinalMTTask));
    if (!task) {
    ...
    task->data = cardinal_alloc(allocator, strlen(file_path) + 1);
    if (task->data) {
