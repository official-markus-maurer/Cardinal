...
bool cardinal_barrier_validation_init(uint32_t max_tracked_accesses, bool strict_mode) {
    ...
    g_validation_context.resource_accesses =
        cardinal_alloc(allocator, sizeof(CardinalResourceAccess) * max_tracked_accesses);
...
void cardinal_barrier_validation_shutdown(void) {
    ...
    CARDINAL_LOG_INFO(
        "[BARRIER_VALIDATION] Shutdown complete. Stats: %u accesses, %u errors, %u race conditions",
...
bool cardinal_barrier_validation_track_access(uint64_t resource_id,
    ...
    for (uint32_t i = 0; i < g_validation_context.access_count; i++) {
        ...
        if (existing->resource_id == resource_id && existing->thread_id != thread_id) {
            ...
            if (access_type == CARDINAL_ACCESS_WRITE ||
                existing->access_type == CARDINAL_ACCESS_WRITE) {
    ...
    // Record the access
    CardinalResourceAccess* access =
        &g_validation_context.resource_accesses[g_validation_context.access_count];
...
bool cardinal_barrier_validation_validate_memory_barrier(const VkMemoryBarrier2* barrier,
    ...
    if (barrier->srcStageMask == 0 || barrier->dstStageMask == 0) {
        CARDINAL_LOG_ERROR("[BARRIER_VALIDATION] Invalid stage mask in memory barrier (thread %u)",
                           thread_id);
        g_validation_errors++;
        valid = false;
    }
    ...
    if ((barrier->srcAccessMask & VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT) &&
        ...
        CARDINAL_LOG_WARN("[BARRIER_VALIDATION] Access mask mismatch with stage mask (thread %u)",
                          thread_id);
    }
...
bool cardinal_barrier_validation_validate_buffer_barrier(const VkBufferMemoryBarrier2* barrier,
    ...
    if (barrier->srcQueueFamilyIndex != barrier->dstQueueFamilyIndex &&
        ...
        CARDINAL_LOG_WARN(
            "[BARRIER_VALIDATION] Inconsistent queue family indices in buffer barrier (thread %u)",
            thread_id);
    }
...
bool cardinal_barrier_validation_validate_image_barrier(const VkImageMemoryBarrier2* barrier,
    ...
    if (barrier->oldLayout == barrier->newLayout && barrier->oldLayout != VK_IMAGE_LAYOUT_GENERAL) {
        CARDINAL_LOG_WARN(
            "[BARRIER_VALIDATION] Unnecessary layout transition (thread %u, image 0x%llx)",
            thread_id, (unsigned long long)image_id);
    }

    // Check for invalid layout transitions
    if (barrier->oldLayout == VK_IMAGE_LAYOUT_UNDEFINED &&
        barrier->newLayout != VK_IMAGE_LAYOUT_PREINITIALIZED && barrier->srcAccessMask != 0) {
        CARDINAL_LOG_ERROR(
            "[BARRIER_VALIDATION] Invalid src access mask for UNDEFINED layout (thread %u)",
            thread_id);
        g_validation_errors++;
        valid = false;
    }
...
bool cardinal_barrier_validation_validate_pipeline_barrier(const VkDependencyInfo* dependency_info,
    ...
    for (uint32_t i = 0; i < dependency_info->memoryBarrierCount; i++) {
        if (!cardinal_barrier_validation_validate_memory_barrier(
                &dependency_info->pMemoryBarriers[i], command_buffer, thread_id)) {
            valid = false;
        }
    }
    ...
    for (uint32_t i = 0; i < dependency_info->bufferMemoryBarrierCount; i++) {
        if (!cardinal_barrier_validation_validate_buffer_barrier(
                &dependency_info->pBufferMemoryBarriers[i], command_buffer, thread_id)) {
            valid = false;
        }
    }
    ...
    for (uint32_t i = 0; i < dependency_info->imageMemoryBarrierCount; i++) {
        if (!cardinal_barrier_validation_validate_image_barrier(
                &dependency_info->pImageMemoryBarriers[i], command_buffer, thread_id)) {
            valid = false;
        }
    }

    CARDINAL_LOG_DEBUG("[BARRIER_VALIDATION] Pipeline barrier validated (thread %u, cmd %p): %u "
                       "memory, %u buffer, %u image barriers",
                       thread_id, (void*)command_buffer, dependency_info->memoryBarrierCount,
...
bool cardinal_barrier_validation_validate_secondary_recording(
    ...
    if (!context->is_recording) {
        CARDINAL_LOG_ERROR(
            "[BARRIER_VALIDATION] Attempting to validate non-recording secondary command buffer");
        g_validation_errors++;
        return false;
    }
    ...
    cardinal_barrier_validation_track_access(
        cmd_buffer_id, CARDINAL_RESOURCE_DESCRIPTOR_SET, CARDINAL_ACCESS_WRITE,
        VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT, VK_ACCESS_2_MEMORY_WRITE_BIT, thread_id,
        context->command_buffer);

    CARDINAL_LOG_DEBUG(
        "[BARRIER_VALIDATION] Secondary command buffer recording validated (thread %u)", thread_id);
    return true;
}
...
bool cardinal_barrier_validation_check_race_condition(uint32_t thread_id1, uint32_t thread_id2) {
    ...
    for (uint32_t i = 0; i < g_validation_context.access_count; i++) {
        ...
        if (access1->thread_id != thread_id1)
            continue;
        ...
        for (uint32_t j = i + 1; j < g_validation_context.access_count; j++) {
            ...
            if (access2->thread_id != thread_id2)
                continue;

            if (access1->resource_id == access2->resource_id &&
                (access1->access_type == CARDINAL_ACCESS_WRITE ||
                 access2->access_type == CARDINAL_ACCESS_WRITE)) {
                CARDINAL_LOG_WARN("[BARRIER_VALIDATION] Race condition detected between threads %u "
                                  "and %u on resource 0x%llx",
...
void cardinal_barrier_validation_get_stats(uint32_t* out_total_accesses,
    ...
    if (out_total_accesses)
        *out_total_accesses = g_total_accesses;
    if (out_validation_errors)
        *out_validation_errors = g_validation_errors;
    if (out_race_conditions)
        *out_race_conditions = g_race_conditions;
}
