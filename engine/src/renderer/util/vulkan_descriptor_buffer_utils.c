...
bool vk_descriptor_buffer_create_manager(const DescriptorBufferCreateInfo* create_info,
    ...
    if (!vulkan_state->context.supports_descriptor_buffer) {
    ...
    vkGetPhysicalDeviceProperties2(vulkan_state->context.physical_device, &props2);
    ...
    VkDeviceSize aligned_layout_size =
        (manager->layout_size + manager->buffer_alignment - 1) & ~(manager->buffer_alignment - 1);
    ...
    if (!vk_allocator_allocate_buffer(manager->allocator, &buffer_info,
                                      &manager->buffer_alloc.buffer, &manager->buffer_alloc.memory,
    ...
    VkResult result = vkMapMemory(manager->device, manager->buffer_alloc.memory, 0, total_size, 0,
                                  &manager->buffer_alloc.mapped_data);
    if (result != VK_SUCCESS) {
        CARDINAL_LOG_ERROR("Failed to map descriptor buffer memory: %d", result);
        vk_allocator_free_buffer(manager->allocator, manager->buffer_alloc.buffer,
    ...
    for (uint32_t i = 0; i < max_bindings; i++) {
        vulkan_state->context.vkGetDescriptorSetLayoutBindingOffsetEXT(
            manager->device, manager->layout, i, &manager->binding_offsets[i]);
    }
...
VkDeviceAddress vk_descriptor_buffer_get_address(const DescriptorBufferManager* manager,
    ...
    VkDeviceAddress base_address =
        vulkan_state->context.vkGetBufferDeviceAddress(manager->device, &address_info);
...
bool vk_descriptor_buffer_update_uniform_buffer(DescriptorBufferManager* manager,
                                                ...
                                                VkDeviceSize range, VulkanState* vulkan_state) {
    ...
    VkDescriptorAddressInfoEXT address_desc = {.sType =
                                                   VK_STRUCTURE_TYPE_DESCRIPTOR_ADDRESS_INFO_EXT,
                                               .address = buffer_address + offset,
                                               .range = range};

    VkDescriptorGetInfoEXT desc_info = {.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_GET_INFO_EXT,
    ...
    // Get descriptor
    vulkan_state->context.vkGetDescriptorEXT(
        manager->device, &desc_info, vulkan_state->context.descriptor_buffer_uniform_buffer_size,
...
bool vk_descriptor_buffer_update_image_sampler(DescriptorBufferManager* manager, uint32_t set_index,
                                               uint32_t binding, uint32_t array_element,
                                               VkImageView image_view, VkSampler vk_sampler,
    ...
    VkDescriptorImageInfo image_info = {
        .sampler = vk_sampler, .imageView = image_view, .imageLayout = image_layout};
    ...
    VkDeviceSize element_offset =
        array_element * vulkan_state->context.descriptor_buffer_combined_image_sampler_size;

    // Get descriptor
    vulkan_state->context.vkGetDescriptorEXT(
        manager->device, &desc_info,
        vulkan_state->context.descriptor_buffer_combined_image_sampler_size,
...
void vk_descriptor_buffer_bind(VkCommandBuffer cmd_buffer, VkPipelineBindPoint pipeline_bind_point,
    ...
    if (!vulkan_state || !vulkan_state->context.vkCmdBindDescriptorBuffersEXT) {
        CARDINAL_LOG_ERROR("Descriptor buffer extension not available");
        return;
    }

    VkDescriptorBufferBindingInfoEXT* binding_infos =
        malloc(set_count * sizeof(VkDescriptorBufferBindingInfoEXT));
    if (!binding_infos) {
    ...
    for (uint32_t i = 0; i < set_count; i++) {
        ...
        VkBufferDeviceAddressInfo address_info = {
            .sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO, .buffer = buffers[i]};
        VkDeviceAddress buffer_address = vulkan_state->context.vkGetBufferDeviceAddress(
            vulkan_state->context.device, &address_info);
    ...
    vulkan_state->context.vkCmdBindDescriptorBuffersEXT(cmd_buffer, set_count, binding_infos);
    free(binding_infos);
}
...
void vk_descriptor_buffer_set_offsets(VkCommandBuffer cmd_buffer,
                                      ...
                                      const VkDeviceSize* offsets, VulkanState* vulkan_state) {
    (void)pipeline_bind_point;
    (void)layout;
    (void)first_set;
    (void)offsets;

    if (!cmd_buffer || !vulkan_state || !vulkan_state->context.supports_descriptor_buffer) {
        CARDINAL_LOG_ERROR("Invalid parameters or descriptor buffer not supported");
        return;
    }

    if (!vulkan_state->context.vkCmdSetDescriptorBufferOffsetsEXT) {
        CARDINAL_LOG_ERROR("vkCmdSetDescriptorBufferOffsetsEXT function not loaded");
        return;
    }

    vulkan_state->context.vkCmdSetDescriptorBufferOffsetsEXT(
