...
pub export fn vk_buffer_find_memory_type(physicalDevice: c.VkPhysicalDevice, typeFilter: u32, properties: c.VkMemoryPropertyFlags) callconv(.c) u32 {
    ...
    while (i < memProperties.memoryTypeCount) : (i += 1) {
        ...
        log.cardinal_log_debug("  Type {d}: heap={d}, flags=0x{X}, typeMatch={s}, propMatch={s}", .{ i, memProperties.memoryTypes[i].heapIndex, memProperties.memoryTypes[i].propertyFlags, if (typeMatches) "yes" else "no", if (propertiesMatch) "yes" else "no" });
        ...
        if (typeMatches and propertiesMatch) {
            ...
            log.cardinal_log_debug("Found suitable memory type: index={d}, heap={d}, size={d} MB", .{ i, memProperties.memoryTypes[i].heapIndex, heapSizeMB });
            return i;
        }
    }
