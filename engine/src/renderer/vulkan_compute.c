...
bool vk_compute_validate_config(VulkanState* vulkan_state, const ComputePipelineConfig* config) {
    ...
    vkGetPhysicalDeviceProperties(vulkan_state->context.physical_device, &properties);
...
bool vk_compute_create_descriptor_layout(VulkanState* vulkan_state,
    ...
    VkResult result =
        vkCreateDescriptorSetLayout(vulkan_state->context.device, &layout_info, NULL, layout);
...
bool vk_compute_create_pipeline(VulkanState* vulkan_state, const ComputePipelineConfig* config,
    ...
    if (!vk_shader_create_module(vulkan_state->context.device, config->compute_shader_path,
                                 &compute_shader)) {
        CARDINAL_LOG_ERROR("[COMPUTE] Failed to load compute shader: %s",
                           config->compute_shader_path);
        return false;
    }
    ...
    result = vkCreatePipelineLayout(vulkan_state->context.device, &pipeline_layout_info, NULL,
                                    &pipeline->pipeline_layout);
    if (result != VK_SUCCESS) {
        CARDINAL_LOG_ERROR("[COMPUTE] Failed to create pipeline layout: %d", result);
        vkDestroyShaderModule(vulkan_state->context.device, compute_shader, NULL);
        return false;
    }
    ...
    VkComputePipelineCreateInfo pipeline_info = {.sType =
    ...
    result = vkCreateComputePipelines(vulkan_state->context.device, VK_NULL_HANDLE, 1,
    ...
    if (config->descriptor_set_count > 0 && config->descriptor_layouts) {
        pipeline->descriptor_layouts =
            malloc(config->descriptor_set_count * sizeof(VkDescriptorSetLayout));
...
void vk_compute_destroy_pipeline(VulkanState* vulkan_state, ComputePipeline* pipeline) {
...
void vk_compute_dispatch(VkCommandBuffer cmd_buffer, const ComputePipeline* pipeline,
    ...
    if (dispatch_info->descriptor_sets && dispatch_info->descriptor_set_count > 0) {
        vkCmdBindDescriptorSets(cmd_buffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                                pipeline->pipeline_layout, 0, dispatch_info->descriptor_set_count,
    ...
    if (dispatch_info->push_constants && dispatch_info->push_constant_size > 0) {
        vkCmdPushConstants(cmd_buffer, pipeline->pipeline_layout, pipeline->push_constant_stages, 0,
                           dispatch_info->push_constant_size, dispatch_info->push_constants);
    }

    // Dispatch compute work
    vkCmdDispatch(cmd_buffer, dispatch_info->group_count_x, dispatch_info->group_count_y,
                  dispatch_info->group_count_z);
}

void vk_compute_memory_barrier(VkCommandBuffer cmd_buffer, const ComputeMemoryBarrier* barrier) {
    ...
    vkCmdPipelineBarrier(cmd_buffer, barrier->src_stage_mask, barrier->dst_stage_mask, 0, 1,
