...
bool vk_create_enhanced_image_barrier(const VkQueueFamilyOwnershipTransferInfo* transfer_info,
    ...
    if (!transfer_info || !out_barrier) {
        CARDINAL_LOG_ERROR(
            "[MAINTENANCE8_SYNC] Invalid parameters for enhanced image barrier creation");
        return false;
    }
    ...
    CARDINAL_LOG_DEBUG(
        "[Thread %u] Enhanced image barrier: queue families %u->%u, stages 0x%llx->0x%llx",
...
bool vk_create_enhanced_buffer_barrier(const VkQueueFamilyOwnershipTransferInfo* transfer_info,
    ...
    if (!transfer_info || !out_barrier) {
        CARDINAL_LOG_ERROR(
            "[MAINTENANCE8_SYNC] Invalid parameters for enhanced buffer barrier creation");
        return false;
    }
    ...
    CARDINAL_LOG_DEBUG(
        "[Thread %u] Enhanced buffer barrier: queue families %u->%u, stages 0x%llx->0x%llx",
        get_current_thread_id(), transfer_info->src_queue_family, transfer_info->dst_queue_family,
        (unsigned long long)transfer_info->src_stage_mask,
        (unsigned long long)transfer_info->dst_stage_mask);
...
bool vk_record_enhanced_ownership_transfer(VkCommandBuffer cmd,
    ...
    if (!cmd || !transfer_info || !vkCmdPipelineBarrier2_func) {
        CARDINAL_LOG_ERROR(
            "[MAINTENANCE8_SYNC] Invalid parameters for enhanced ownership transfer");
        return false;
    }
    ...
    if (transfer_info->use_maintenance8_enhancement &&
        ...
        // Use maintenance8 enhancement for meaningful stage masks in queue family ownership
        // transfers
        dependency_info.dependencyFlags =
            VK_DEPENDENCY_QUEUE_FAMILY_OWNERSHIP_TRANSFER_USE_ALL_STAGES_BIT_KHR;
        CARDINAL_LOG_DEBUG("[MAINTENANCE8_SYNC] Using maintenance8 enhanced synchronization for "
                           "queue family ownership transfer");
    } else {
        dependency_info.dependencyFlags = 0;
        if (transfer_info->src_queue_family != transfer_info->dst_queue_family) {
            CARDINAL_LOG_DEBUG("[MAINTENANCE8_SYNC] Using standard synchronization for queue "
    ...
    if (!cardinal_barrier_validation_validate_pipeline_barrier(&dependency_info, cmd,
                                                               get_current_thread_id())) {
        CARDINAL_LOG_WARN("[MAINTENANCE8_SYNC] Pipeline barrier validation failed for enhanced "
                          "ownership transfer");
    }
    ...
    CARDINAL_LOG_INFO("[MAINTENANCE8_SYNC] Recorded enhanced ownership transfer: %u image "
                      "barriers, %u buffer barriers, maintenance8=%s",
 ...
 */
bool vk_create_queue_family_transfer_info(uint32_t src_queue_family, uint32_t dst_queue_family,
    ...
    if (!out_transfer_info) {
        CARDINAL_LOG_ERROR(
            "[MAINTENANCE8_SYNC] Invalid output parameter for transfer info creation");
        return false;
    }
    ...
    out_transfer_info->use_maintenance8_enhancement =
        supports_maintenance8 && (src_queue_family != dst_queue_family);

    CARDINAL_LOG_DEBUG(
        "[MAINTENANCE8_SYNC] Created queue family transfer info: %u -> %u, maintenance8=%s",
