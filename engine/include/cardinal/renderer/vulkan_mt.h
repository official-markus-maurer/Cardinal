...
#ifdef _WIN32
#include <windows.h>
typedef HANDLE cardinal_thread_handle_t;
typedef DWORD cardinal_thread_id_t;
typedef CRITICAL_SECTION cardinal_mutex_t;
typedef CONDITION_VARIABLE cardinal_cond_t;
#else
#include <pthread.h>
typedef pthread_t cardinal_thread_handle_t;
typedef pthread_t cardinal_thread_id_t;
...
typedef struct CardinalThreadCommandPool {
  ...
  cardinal_thread_id_t thread_id;     // Thread ID that owns this pool
...
typedef struct CardinalMTCommandManager {
  ...
  CardinalThreadCommandPool
      thread_pools[CARDINAL_MAX_MT_THREADS]; // Per-thread command pools
...
typedef struct CardinalSecondaryCommandContext {
  ...
  VkCommandBufferInheritanceInfo
      inheritance;       // Inheritance info for secondary buffer
...
typedef struct CardinalMTSubsystem {
  ...
  cardinal_thread_handle_t
      worker_threads[CARDINAL_MAX_MT_THREADS]; // Worker threads
 ...
 */
CardinalThreadCommandPool *
cardinal_mt_get_thread_command_pool(CardinalMTCommandManager *manager);
 ...
 * @return true on success, false on failure
 */
bool cardinal_mt_allocate_secondary_command_buffer(
    CardinalThreadCommandPool *pool, CardinalSecondaryCommandContext *context);
 ...
 * @return true on success, false on failure
 */
bool cardinal_mt_end_secondary_command_buffer(
    CardinalSecondaryCommandContext *context);
...
void cardinal_mt_execute_secondary_command_buffers(
    ...
    CardinalSecondaryCommandContext *secondary_contexts, uint32_t count);
 ...
 * @return true on success, false on failure
 */
bool cardinal_mt_subsystem_init(VulkanState *vulkan_state,
 ...
 */
CardinalMTTask *cardinal_mt_create_texture_load_task(
    const char *file_path, void (*callback)(void *data, bool success));
 ...
 */
CardinalMTTask *
cardinal_mt_create_mesh_load_task(const char *file_path,
 ...
 * @return Pointer to the created task, or NULL on failure
 */
CardinalMTTask *cardinal_mt_create_command_record_task(
    void (*record_func)(void *data), void *user_data,
    void (*callback)(void *data, bool success));
 ...
 */
bool cardinal_mt_thread_ids_equal(cardinal_thread_id_t thread1,
                                  cardinal_thread_id_t thread2);
 ...
 */
bool cardinal_mt_cond_wait_timeout(cardinal_cond_t *cond,
                                   cardinal_mutex_t *mutex,
                                   uint32_t timeout_ms);
 ...
 * @return true on success, false on failure
 */
bool cardinal_mt_thread_create(cardinal_thread_handle_t *thread,
 ...
 * @return true on success, false on failure
 */
bool cardinal_mt_thread_join(cardinal_thread_handle_t thread);
 ...
 * @return true on success, false on failure
 */
bool cardinal_mt_thread_detach(cardinal_thread_handle_t thread);
