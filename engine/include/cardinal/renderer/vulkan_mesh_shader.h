 ...
 * @brief Vulkan mesh shader pipeline management and GPU-driven rendering support
 * 
 * This module provides mesh shader pipeline creation, management, and GPU-driven
 * rendering capabilities using VK_EXT_mesh_shader extension.
 */
...
typedef struct MeshShaderPipelineConfig {
    ...
    const char* task_shader_path;        /**< Path to task shader SPIR-V file (optional) */
...
typedef struct MeshShaderPipeline {
    ...
    // Descriptor set layouts (needed for pipeline validation with descriptor buffers)
    VkDescriptorSetLayout descriptor_layout; /**< Descriptor set layout for mesh shader (Set 0) */
    VkDescriptorSetLayout fragment_descriptor_layout; /**< Descriptor set layout for fragment shader (Set 1) */
    
    // Descriptor buffer resources
    VkBuffer mesh_descriptor_buffer;     /**< Descriptor buffer for mesh shader data (Set 0) */
    VkDeviceMemory mesh_descriptor_memory; /**< Memory for mesh descriptor buffer */
    VkBuffer fragment_descriptor_buffer; /**< Descriptor buffer for fragment shader data (Set 1) */
    VkDeviceMemory fragment_descriptor_memory; /**< Memory for fragment descriptor buffer */
    VkDeviceSize mesh_descriptor_offset; /**< Offset into mesh descriptor buffer */
    VkDeviceSize fragment_descriptor_offset; /**< Offset into fragment descriptor buffer */
    
    // Placeholder resources for validation compliance
    VkBuffer placeholder_buffer;         /**< Placeholder buffer for unbound descriptors */
    VkDeviceMemory placeholder_buffer_memory; /**< Memory for placeholder buffer */
...
typedef struct MeshShaderMaterial {
    ...
    uint32_t metallicRoughnessTextureIndex; /**< uint metallicRoughnessTextureIndex */
 ...
 * 
 * Updates both mesh shader descriptor buffer (Set 0) and fragment shader descriptor buffer (Set 1)
 * with the provided buffers and textures.
 * 
 * @param vulkan_state Vulkan state object
 ...
 */
bool vk_mesh_shader_create_uniform_buffer(VulkanState* vulkan_state,
                                           MeshShaderPipeline* pipeline,
                                           const MeshShaderUniformBuffer* uniform_data,
 ...
 * @return true on success, false on failure
 */
bool vk_mesh_shader_update_uniform_buffer(VulkanState* vulkan_state,
                                           VkBuffer uniform_buffer,
                                           VkDeviceMemory uniform_memory,
                                           const MeshShaderUniformBuffer* uniform_data);
