...
pub fn draw_scene_manager_panel(state: *EditorState, allocator: std.mem.Allocator) void {
    if (state.ui.show_scene_manager) {
        const open = c.imgui_bridge_begin("Scene Manager", &state.ui.show_scene_manager, 0);
        ...
        if (open) {
            ...
            if (c.imgui_bridge_begin_child("##scene_list", 0, 0, true, 0)) {
                if (state.ui.available_scenes.items.len == 0) {
                    c.imgui_bridge_text_disabled("No scenes found.");
                } else {
                    for (state.ui.available_scenes.items) |scene_name| {
                        ...
                        if (c.imgui_bridge_is_item_clicked(1)) {
                            const len = @min(scene_name.len, state.ui.scene_context_menu_name.len - 1);
                            @memcpy(state.ui.scene_context_menu_name[0..len], scene_name[0..len]);
                            state.ui.scene_context_menu_name[len] = 0;

                            @memset(&state.ui.rename_scene_buffer, 0);
                            @memcpy(state.ui.rename_scene_buffer[0..len], scene_name[0..len]);
            ...
            if (state.ui.open_rename_popup) {
                c.imgui_bridge_open_popup("Rename Scene");
                state.ui.open_rename_popup = false;
            }

            if (state.ui.open_delete_popup) {
                c.imgui_bridge_open_popup("Delete Scene");
                state.ui.open_delete_popup = false;
            }
...
fn rename_scene(state: *EditorState, allocator: std.mem.Allocator) void {
    _ = allocator;
    const old_name_len = std.mem.indexOf(u8, &state.ui.scene_context_menu_name, &[_]u8{0}) orelse state.ui.scene_context_menu_name.len;
    if (old_name_len == 0) return;
    const old_name = state.ui.scene_context_menu_name[0..old_name_len];

    const new_name_len = std.mem.indexOf(u8, &state.ui.rename_scene_buffer, &[_]u8{0}) orelse state.ui.rename_scene_buffer.len;
    if (new_name_len == 0) return;
    const new_name = state.ui.rename_scene_buffer[0..new_name_len];
...
fn delete_scene(state: *EditorState, allocator: std.mem.Allocator) void {
    _ = allocator;
    const name_len = std.mem.indexOf(u8, &state.ui.scene_context_menu_name, &[_]u8{0}) orelse state.ui.scene_context_menu_name.len;
    if (name_len == 0) return;
    const name = state.ui.scene_context_menu_name[0..name_len];
